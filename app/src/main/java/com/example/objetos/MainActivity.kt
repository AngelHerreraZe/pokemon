...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        maincontext = this
/*
        var Angel: Person = Person("Angel", "A3216596456")
        var Anonimo: Person = Person()
        println(Angel.alive)
        println(Angel.name)
        println(Angel.passport)
 ...
 */
        ...
        btFight.setOnClickListener { fight(waterPok, firePok) }
    ...
    private fun fight(p1: Pokemon, p2: Pokemon){
        ...
        while(p1.getLife() > 0 && p2.getLife() > 0){
            ...
            text += """${p1.getName()} (${p1.getLife().toInt()}) Vs ${p2.getName()} (${p2.getLife().toInt()})"""
            if (p2.getLife() > 0){
                text += "\n${p2.getName()} ataca!"
                p2.attack()
